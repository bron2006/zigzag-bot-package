    ...
    def on_ready(results):
        try:
            ...
            if not success_df or df is None or df.empty or not success_daily or daily_df is None or daily_df.empty:
                ...
                final_deferred.callback({"error": err_msg})
                return
            ...
            response_data = {
                "pair": norm_pair, "price": current_price,
            ...
            if user_id != 0: # Не зберігаємо історію для сканера
                add_signal_to_history({'user_id': user_id, 'pair': norm_pair, 'price': current_price, 'bull_percentage': int(score)})

            try:
                r = get_redis()
                if r:
                    r.set(f"signal:{norm_pair}:{period}", json.dumps(response_data, ensure_ascii=False), ex=60 * 60 * 2) # TTL 2 години
            except Exception:
