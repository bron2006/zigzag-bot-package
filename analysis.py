...
from api_clients import get_binance_client # <-- Новий, безпечний імпорт
...
def get_market_data(pair, tf, asset, limit=300, force_refresh=False):
    ...
    try:
        ...
        if asset == 'crypto':
            ...
            if not binance:
                logger.error("Binance client is not available.")
                return pd.DataFrame()
            ...
            if resolution.isdigit():
                delta_seconds = limit * int(resolution) * 60
            else:
                delta_seconds = limit * 24 * 3600
...
# (Скопіюйте сюди решту функцій з попередніх версій файлу)
