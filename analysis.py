...
def _prepare_features(df: pd.DataFrame):
    ...
    try:
        ...
        latest = df.tail(1)
        features = [
            latest["ATRr_14"].values[0], latest["ADX_14"].values[0],
            latest["RSI_14"].values[0], latest["EMA_50"].values[0],
...
def get_market_data(client, symbol_cache, norm_pair, period, count):
    ...
    req = ProtoOAGetTrendbarsReq(ctidTraderAccountId=client._client.account_id, symbolId=symbol_details.symbolId, period=tf_proto, fromTimestamp=from_ts, toTimestamp=now)
