...
def _calculate_binary_signal(df: pd.DataFrame) -> Dict:
    """
    Нова логіка аналізу для бінарних опціонів.
    Шукає збіг умов від Bollinger Bands та Stochastic Oscillator.
    """
    ...
    if is_oversold and is_touching_lower_band and is_bullish_candle:
        ...
        reasons.append("Ціна торкнулася або пробила нижню лінію Боллінджера")
    ...
    if is_overbought and is_touching_upper_band and is_bearish_candle:
        ...
        reasons.append("Ціна торкнулася або пробила верхню лінію Боллінджера")
...
def get_api_detailed_signal_data(client, symbol_cache, symbol: str, user_id: int, timeframe: str = "5m") -> Deferred:
    ...
    def on_data_ready(df: pd.DataFrame):
        try:
            ...
            response_data = {
                ...
                "bull_percentage": int(analysis.get('stochastic', 50)), # Використовуємо стохастик як score
    ...
    # Для бінарних опціонів нам потрібен лише один, короткий таймфрейм
