...
def get_main_menu_kb() -> InlineKeyboardMarkup:
    ...
    for key, text in scanner_map.items():
        ...
        keyboard.append([InlineKeyboardButton(f"{status_icon} Сканер {text}", callback_data=f"toggle_scanner_{key}")])
...
def get_forex_sessions_kb(expiration: str) -> InlineKeyboardMarkup:
    ...
    for session_name in FOREX_SESSIONS:
        ...
        keyboard.append([InlineKeyboardButton(text, callback_data=f"session_forex_{expiration}_{session_name}")])
...
def get_assets_kb(asset_list: list, category: str, expiration: str) -> InlineKeyboardMarkup:
    ...
    for asset in asset_list:
        row.append(InlineKeyboardButton(asset, callback_data=f"analyze_{expiration}_{asset.replace('/', '')}"))
        if len(row) == 2:
            keyboard.append(row)
            row = []
    if row:
...
def _format_signal_message(result: dict, expiration: str) -> str:
    ...
    parts = [
        f"📈 *Сигнал для {pair} (Експірація: {expiration})*",
        f"**Прогноз:** {verdict}",
...
def button_handler(update: Update, context: CallbackContext) -> None:
    ...
    if action == "main":
        ...
        def on_error(failure):
            ...
            context.bot.send_message(chat_id, f"❌ Помилка: {failure}")
