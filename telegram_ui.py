...
def get_pairs_kb(session: str) -> InlineKeyboardMarkup:
    ...
    row = []
    for pair in pairs:
        # Важливо: callback_data не повинен містити '/', бо це може ламати логіку
...
# --- ОБРОБНИКИ (з вашого файлу) ---
def start(update: Update, context: CallbackContext) -> None:
    """Створює головну, постійну клавіатуру з кнопкою МЕНЮ."""
    keyboard = [["МЕНЮ"]]
...
def reset_ui(update: Update, context: CallbackContext) -> None:
    """При будь-якому текстовому повідомленні, крім 'МЕНЮ', не робить нічого (щоб не спамити)."""
    pass

def menu(update: Update, context: CallbackContext) -> None:
    """Обробляє натискання на кнопку 'МЕНЮ' і показує inline-меню."""
    try:
...
def _format_signal_message(result: dict) -> str:
    # Ця функція залишається без змін, як у вашому файлі
    ...
    reasons = result.get('reasons', []); price_str = f"{price:.5f}" if price else "N/A"
...
def button_handler(update: Update, context: CallbackContext) -> None:
    ...
    if data == "main_menu":
        ...
        if symbol not in state.symbol_cache:
            query.answer(text=f"⚠️ Символ {symbol} не знайдено в кеші.", show_alert=True)
            return
        ...
        def on_error(failure):
            logger.error(f"❌ Помилка при отриманні сигналу для {symbol}: {failure}")
            query.edit_message_text(text=f"❌ Виникла помилка під час аналізу {symbol}.", reply_markup=get_forex_sessions_kb())
        
        def do_analysis():
            # Передаємо клієнт та символ в аналітичну функцію
            d = get_api_detailed_signal_data(state.client, symbol)
