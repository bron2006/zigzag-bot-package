...
def button_handler(update: Update, context: CallbackContext):
    ...
    if data == 'main_menu':
        ...
        display_ticker = data[len("analyze_"):]          # зберігаємо красивий вигляд для UI
        api_ticker = display_ticker.replace("/", "")      # для API/кешу треба без слеша
        ...
        query.edit_message_text(f"⏳ Аналізую {display_ticker}...")

        def on_analysis_done(analysis_data):
            # Безпечний виклик з твістед-потоку
            reactor.callFromThread(_send_analysis_result, context, query, analysis_data, display_ticker)

        def on_analysis_error(failure):
            logger.error(f"Analysis failed for {display_ticker}: {failure}")
            error_text = str(getattr(failure, "value", failure))
            error_data = {"error": error_text}
            reactor.callFromThread(_send_analysis_result, context, query, error_data, display_ticker)

        d = get_api_detailed_signal_data(client, api_ticker, user_id=user_id)
...
def _send_analysis_result(context, query, analysis_data, ticker):
    if "error" in analysis_data:
        ...
        try:
            ...
            try:
                context.bot.send_message(chat_id=query.message.chat_id, text=msg, reply_markup=kb)
            except Exception:
                pass
        return
    ...
    try:
        ...
        try:
            context.bot.send_message(chat_id=query.message.chat_id, text=msg, parse_mode='Markdown', reply_markup=kb)
        except Exception:
            pass
...
def register_handlers(dispatcher, client):
    ...
    dispatcher.add_handler(MessageHandler(Filters.text & Filters.regex(r"^МЕНЮ$"), menu_command))
