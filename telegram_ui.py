...
from state import state # Тепер це працює, бо state - це модуль
...
def menu(update: Update, context: CallbackContext) -> None:
    # ... (ваш код без змін)
    try: context.bot.delete_message(chat_id=update.message.chat_id, message_id=update.message.message_id)
...
def _format_signal_message(result: dict) -> str:
    ...
    pair = result.get('pair', 'N/A'); price = result.get('price', 0); verdict = result.get('verdict_text', 'Не вдалося визначити.')
    support = result.get('support'); resistance = result.get('resistance'); reasons = result.get('reasons', [])
