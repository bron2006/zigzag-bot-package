...
2026-08-09 напряму з живим терміналом Binomo, включно з реальним
розміщенням demo-угоди (`Bitcoin (OTC)`, ₴40, 5 хв) на навчальному
рахунку — вибір активу, введення суми, степпер часу експірації і клік
"вгору" підтверджено робочими. У процесі виявлено і виправлено два
нетривіальні баги, які не спливли б при суто читанні DOM:

- Поле суми — кастомний Angular-компонент з маскою; звичайний `.fill()`
  тихо спотворював значення (замість "40.00" ставало "40.4"). Замінено на
  посимвольне введення з перевіркою результату після вводу.
- Це ж поле суми не приймає десяткову крапку — символ `.` просто
  відкидається, а решта цифр склеюється (замість "40.00" виходило
  "4000"). Тепер вводиться ціле число.
- Парсинг балансу (`350 712,00 ₴`, кома як десятковий роздільник) раніше
  просто видаляв кому, завищуючи баланс у 100 разів — це напряму впливало
  б на розмір ставки (`BINOMO_STAKE_PERCENT` від балансу). Виправлено в
  `_parse_numeric_text()`.
- Кнопка "+" для часу експірації спершу шукалась за SVG-іконкою
  (`use[href=...]`), що не спрацювало наживо; замінено на пошук за
  позицією в контейнері (мінус/плюс завжди в такому порядку).

**2026-08-10**: `--correlation-check` реально не записував жодної ціни
Binomo (`binomo_entry` завжди `None`) — виявлено дві причини й обидві
виправлено, перевірено наживо (видимий браузер + скріншот, реальна ціна
`93353.9125` для Bitcoin (OTC)):

- Ціну Binomo справді неможливо прочитати з DOM (малюється лише в
  `<canvas>`, без жодного тексту) — але вона йде окремим WebSocket
  (`wss://as.binomo.com/`, кадри виду
  `{"data":[{"assets":[{"rate": ..., "ric": ...}]}]}`). `_read_binomo_price()`
  тепер підписується на цей WebSocket через `page.on("websocket")` і кешує
  останній тик, замість шукати неіснуючий DOM-елемент.
- `--correlation-check` взагалі ніколи не перемикав графік на потрібний
  актив перед читанням ціни — читав що завгодно було відкрите на екрані.
  Тепер `_select_asset()` (та сама логіка, що й у `place_binary_trade`)
  викликається перед кожним читанням ціни, і кеш WebSocket-ціни
  очищується при кожному перемиканні активу, щоб стара ціна від
  попереднього активу не потрапила в лог помилково.

Також додано exponential backoff (5с→60с) на reconnect SSE-потоку сигналів
(`_stream_signals`) — обриви з'єднання це нормально, головне —
самовідновлення без зайвого навантаження на сервер.

Лишились відомі прогалини, задокументовані в докстрінгу модуля:
...
- `trade_history_row` — розмітка самого рядка історії угод не перевірена
  наживо (`read_trade_result` ще не тестувався), лишився scoped
  `:has-text()`-фолбек всередині перевіреного контейнера.
